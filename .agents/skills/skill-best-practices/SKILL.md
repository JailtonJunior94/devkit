...
## Input
- Identify whether the task is creating a new skill, restructuring an existing skill, or improving metadata and discoverability.
- Read `assets/skill-template.md` before drafting or restructuring the main `SKILL.md`.
- Read `references/checklist.md` before final validation.
...
*   **Structure Drift:** If an existing skill uses non-standard folders or top-level docs, move reusable materials into `assets/` or `references/` and delete human-oriented files.
